...
async def get_all_notes(db:AsyncSession = Depends(get_db), current_user = Depends(get_current_user)):
    return await note_service.get_all_notes(db)

@router.get("/{note_id}", response_model=NoteInDB)
async def get_note(note_id: str, db: AsyncSession = Depends(get_db), current_user = Depends(get_current_user)):
...
async def delete_note(note_id: str, db: AsyncSession = Depends(get_db), current_user = Depends(get_current_user)):
